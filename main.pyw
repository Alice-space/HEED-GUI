...
# setting interval to zero will NOT make it faster, see README for more info
...
class Orchestrator:
    ...
    def __init__(self):
        ...
        auth['username'] = simpledialog.askstring(
            'Auth', f'请先阅读README.md\n\nUsername:') or ''
        if '@' in auth['username']:
            auth['username'], _, auth['channel'] = auth['username'].partition(
                '@')
        else:
            auth['channel'] = None
        auth['password'] = simpledialog.askstring(
            'Auth', 'Password:', show='*') or ''
    ...
    def manual_select(self):
        ...
        if cid and cid in self.courses_display:
            ...
            if course['elected_cnt'] < course['volume_cnt']:
                self.log(
                    'info', f'manual select {course["name"]} {course["classid"]}')
                ...
                if bot.status == 'idle':
                    ...
                    self.log(
                        'warning', f'cannot select bot status is {bot.status}')
            else:
                info = (course['name'], course['classid'])
                if info in self.wishlist:
                    self.log(
    ...
    def add_bot(self):
        ...
        def do_add_bot():
            ...
            try:
                ...
                self.log(
                    'warning', f'{bot.name} auth failed {type(e)} {str(e)}')
    ...
    def refresh(self):
        ...
        if self.auto_on:
            self.auto_id = tk.after(tweak_intv_time(
                self.INTERVAL_MS), self.refresh)
    ...
    def check_wish(self, bot, courses):
        ...
        for course in courses:
            ...
            if in_wishlist and course['elected_cnt'] < course['volume_cnt']:
                self.log(
                    'info', f'auto select {course["name"]} {course["classid"]}')

                shadowed_list = [
                    (name, classid) for name, classid in self.wishlist if name == course['name']]
                self.wishlist_busy.update(shadowed_list)
                self.log(
                    'debug', f'shadowing {" ".join(map(repr, shadowed_list))}')
                self.update_wish_var()
                ...
                def callback(ok, reason):
                    if ok:
                        ...
                        self.notifier.report_failed_choice(
                            course['name'], reason)
        ...
        def render_worker():
            ...
            while True:
                ...
                if item is None:  # render list
                    sorted_bots = [x for x in self.bots if x.status !=
                                   'dead'] + [x for x in self.bots if x.status == 'dead']
                    li_items = [self, self.notifier] + sorted_bots
                    li_var.set(
                        tuple(f'{x.name} [{x.status}]' for x in li_items))
                    ...
                    with cs.log.lock:
                        ...
                        for i in range(text_lid_end, cs.log.lid_end):
                            try:
                                ...
                                text_lid_end += 1
                                text.insert(
                                    'end', f'{time.strftime("%m-%d %H:%M:%S", time.localtime(tim))} ', 'time')
    ...
    def init_main_window(self):
        ...
        notif_var = StringVar(
            tk, value='on' if self.notifier.status == 'on' else 'off')
        ...
        Button(btnpanel, text='Add Bot',
               command=self.add_bot).grid(row=0, column=0)
        Checkbutton(btnpanel, text='Captcha', variable=auto_captcha_var,
                    onvalue='on', offvalue='off').grid(row=0, column=1)
        Button(btnpanel, text='Refresh',
               command=self.refresh).grid(row=0, column=2)
        Checkbutton(btnpanel, text='Auto', command=ref_changed,
                    variable=auto_refresh_var, onvalue='on', offvalue='off').grid(row=0, column=3)
        ...
        def change_time_config(e):
            ...
            self.log(
                'info', f'change to interval {self.INTERVAL_MS}ms timeout {int(TIMEOUT_S * 1000)}ms')
        ...
        Checkbutton(btnpanel, text='Verbose', command=verbose_changed,
                    variable=verbose_var, onvalue='on', offvalue='off').grid(row=0, column=8)
        ...
        Checkbutton(btnpanel, text='Notif', command=notif_changed,
                    variable=notif_var, onvalue='on', offvalue='off').grid(row=0, column=9)
    ...
    def update_wish_var(self):
        self.wish_var.set(tuple(
            f'{"* " if course in self.wishlist_busy else ""}{course[0]} {course[1]}' for course in self.wishlist))
