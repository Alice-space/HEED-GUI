...
class ElectiveBot:
        ...
        if '<title>帮助-总体流程</title>' not in res.text:
            if '/scnStAthVef.jsp/' in res.text:
                if not auth['channel']:
                    ...
                    sida = res.text.partition(
                        '/ssoLogin.do?sida=')[2].partition('&')[0]
                    assert sida.isalnum(), 'invalid sida'

                    self.log('debug', 'choose channel %s for sida %s' %
                             (auth['channel'], sida))
    ...
    def proc_course_elem(self, rows):
        for row in rows:
            ...
            selectbtn = (row.select(
                'a[href^="/elective2008/edu/pku/stu/elective/controller/supplement/electSupplement.do"]') or [None])[0]

            volume_cnt, _, elected_cnt = row.select('td span[id^="electedNum"]')[
                0].get_text(strip=True).partition(' / ')
            if '/' in elected_cnt:
    ...
    @staticmethod
    def detect_err(soup):
        fatal_err_elems = soup.select(
            'td[background="/elective2008/resources/images/11-1.png"] td.black')
        if fatal_err_elems:
    ...
    def loop_(self, url=None):
        ...
        courses = list(self.proc_course_elem(soup.select(
            'tr.datagrid-all, tr.datagrid-odd, tr.datagrid-even')))
        ...
        if not next_link:
            ...
            try:
                next = self.loop_(
                    f'https://elective.pku.edu.cn{next_link.attrs["href"]}')
                return courses + next
    ...
    def enter_captcha(self, tk, should_autoinput=False):
        if should_autoinput:
            ...
            try:
                ...
                (result, id) = captcha.recognize(img)
                self.log('debug', f'recognized captcha {result}')
                assert self.verify_captcha(result), 'incorrect captcha'
            except Exception as e:
                self.log('info', captcha.report_error(id))
                raise RuntimeError(
                    f'captcha recognize error {type(e)} {str(e)}')
            else:
